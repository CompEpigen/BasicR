...
vignette: >
  %\VignetteIndexEntry{Importing and Wrangling}
...
The current directory should be `/BasicR/vignettes` If you need to change your working directory to `/BasicR`, you can use the function `setwd`
...
Use the `dir` function to list spreadsheets available in the `/BasicsR/data` folder. Be careful of relative paths! 
Use `../` for the parent (`/BasicsR') directory, and then `data`.
...
Lload all the tidyverse packages at once by installing and loading the __tidyverse__ package.
And then use `read_csv` to read-in the `murders.csv`
...
Show three variables (state, region, rate) for states that have murder rates below 0.71. For such an operation, we can use the pipe `%>%`. The code looks like this:
