...
#hypo <- file[diffmeth.p.val<0.00001 & mean.diff< -0.4,]
...
for (dataset in data){
genome(dataset) <- rep("hg19", length(genome(dataset)))
background2 <- background[sample(1:length(background), length(dataset)),]
result <- annotate_regions(dataset, annotations=annots_gr, minoverlap = 1L, ignore.strand = TRUE, quiet = FALSE)
...
for (dataset in data){
  genes <- seq2gene(dataset, tssRegion = c(-3000, 3000), flankDistance = 3000, TxDb=txdb)
